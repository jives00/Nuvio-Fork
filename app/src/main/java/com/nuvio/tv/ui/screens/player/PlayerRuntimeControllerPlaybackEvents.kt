...
internal fun PlayerRuntimeController.emitScrobblePause(progressPercent: Float? = null) {
    ...
    scope.launch(kotlinx.coroutines.NonCancellable) {
        ...
        directScrobbleService.stop(item, percent, paused = true) // [FORK]
...
internal fun PlayerRuntimeController.emitSeekScrobbleRestart(progressPercent: Float) {
    ...
    scope.launch {
        ...
        directScrobbleService.stop(item, progressPercent, paused = true) // [FORK]
        if (isPlaybackCurrentlyPlaying()) {
            val resumePercent = currentPlaybackProgressPercent()
            trackingScrobbleCoordinator.scrobbleSeek(
                action = TrackingScrobbleAction.START,
                event = TrackingScrobbleEvent(item, resumePercent.toDouble())
            )
            directScrobbleService.start(item, resumePercent) // [FORK]
        }
    }
}
